...
public class NewsPresenter extends BasePresenterImpl implements IHeadLine.newspresenter
{
    private IHeadLine.Views mViews;
    private Gson mGson;
    private CacheUtil mUtil;

    public NewsPresenter(Context context, IHeadLine.Views views)
    {
        mViews = views;
        mGson = new Gson();
    ...
    @Override
    public void loadingDataFromNet(String msg,int p)
    {
        Subscription s = ApiManager.getInstance().getHeaderService()
                ...
                .subscribe(new Subscriber<HeadLine>()
                {
                    ...
                    public void onNext(HeadLine line)
                    {
                        ...
                        mUtil.put(Config.NEWS_COOKIE, mGson.toJson(line));
                    }
                });
        addSubcrible(s);
    }
